...
04 62 98 27 23 09 70 98 73 93 38 53 60 04 23
""".strip()
triangle = [[int(x) for x in l.split()] for l in triangle.split('\n')]

# initialize lists for storing intermediate path lengths
path_lengths = [[0] * len(l) for l in triangle]


def get_parents(row, column):
    parent1 = path_lengths[row - 1][column - 1] if column > 0 else 0
    parent2 = path_lengths[row - 1][column] if column < len(triangle[row - 1]) else 0
